...
export const AdmissionsWizard: React.FC<{ onComplete?: (student: Student) => void }> = ({ onComplete }) => {
  const { classArms, registerStudent, getNextAdmissionNumber, parents, students } = useSchoolData();

  const nextSerialId = React.useMemo(() => getNextAdmissionNumber(), [getNextAdmissionNumber]);
  const [admissionNumberInput, setAdmissionNumberInput] = useState('');

  React.useEffect(() => {
    if (!admissionNumberInput) {
      setAdmissionNumberInput(nextSerialId);
    }
  }, [nextSerialId]);
  ...
  const handleSelectParent = (pId: string) => {
    ...
    const found = parents.find(p => p.id === pId);
    if (found) {
      setParentName(found.fullName || found.name || '');
      setParentPhone(found.phoneNumber || found.phone || '');
  ...
  const handleFinish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newStudent = await registerStudent({
        admissionNumber: admissionNumberInput || nextSerialId,
        ...
        bloodGroup,
        genotype,
        parentId: parentMode === 'EXISTING' && selectedParentId ? selectedParentId : undefined,
      ...
      console.error('Admission registration failed:', err);
      const msg = err?.message || 'Failed to complete admission and save student record. Please try again.';
      setSubmitError(msg);
    } finally {
      ...
      <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 rounded-2xl p-8 border border-slate-200 dark:border-slate-800 shadow-md text-center space-y-6">
        ...
        <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-left max-w-md mx-auto space-y-2 text-xs">
          <div className="flex justify-between items-center">
            ...
            <span className="font-mono-tabular font-bold text-amber-900 dark:text-amber-400 bg-amber-500/15 px-2 py-0.5 rounded text-xs">
          ...
          <div className="flex justify-between">
            <span className="text-slate-500 dark:text-slate-400">Parent / Guardian:</span>
            <span className="text-slate-700 dark:text-slate-300">{registeredStudent.parentName} ({registeredStudent.parentPhone})</span>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
          <button
            ...
            onClick={() => {
              ...
              setParentPhone('');
              setParentEmail('');
              setAdmissionNumberInput(getNextAdmissionNumber());
            }}
            className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-bold text-xs transition-colors cursor-pointer"
          >
          ...
          {onComplete && (
            <button
              type="button"
              onClick={() => onComplete(registeredStudent)}
              className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs shadow-xs transition-all cursor-pointer"
            >
              View in Student Directory &rarr;
            </button>
          )}
        </div>
      </div>
      ...
      <form onSubmit={handleFinish} className="p-6 sm:p-8 space-y-6">
        {step === 1 && (
          <div className="space-y-4 animate-in fade-in duration-150">
            {/* Auto-generated Serial Student ID Badge */}
            <div className="p-4 rounded-2xl bg-linear-to-r from-amber-500/10 via-amber-500/5 to-transparent border border-amber-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[11px] font-bold text-amber-900 dark:text-amber-300 uppercase tracking-wider flex items-center gap-2">
                    <span>Auto-Assigned Serial Student ID</span>
                    <span className="text-[9px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 font-bold">
                      Next in Sequence
                    </span>
                  </div>
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">
                    Calculated automatically from the latest enrolled student in the database.
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  value={admissionNumberInput || nextSerialId}
                  onChange={e => setAdmissionNumberInput(e.target.value)}
                  className="font-mono-tabular font-bold text-sm text-amber-950 dark:text-amber-200 bg-white dark:bg-slate-800 px-3.5 py-2 rounded-xl border border-amber-300 dark:border-amber-700 focus:outline-none focus:border-amber-500 shadow-2xs w-44 text-center"
                />
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                  ...
                  <span>Parent / Legal Guardian Entity</span>
                </div>
              ...
              {parentMode === 'EXISTING' ? (
                <div className="space-y-3">
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-600 dark:text-slate-400 mb-1">
                      Choose Registered Parent Profile *
                    </label>
                    <select
                      value={selectedParentId}
                      onChange={e => handleSelectParent(e.target.value)}
                      required
                      className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:border-amber-500 cursor-pointer"
                    >
                      <option value="">-- Choose Existing Family --</option>
                      {parents.map(p => {
                        const wardCount = (p.wardIds || []).length;
                        return (
                          <option key={p.id} value={p.id}>
                            {p.fullName || p.name} ({p.email || p.phoneNumber || 'Registered'}) {wardCount > 0 ? `— ${wardCount} enrolled ward(s)` : ''}
                          </option>
                        );
                      })}
                    </select>
                  ...
                  {selectedParentId && (
                    (() => {
                      ...
                      if (!sel) return null;
                      const enrolledWards = students.filter(
                        s => s.parentId === sel.id || (sel.email && s.parentEmail.toLowerCase() === sel.email.toLowerCase())
                      );
                      return (
                        <div className="p-3.5 rounded-xl bg-emerald-500/10 dark:bg-emerald-950/30 border border-emerald-500/20 text-xs space-y-2 animate-in fade-in duration-150">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5 font-bold text-emerald-800 dark:text-emerald-300">
                              <ShieldCheck className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                              <span>Foreign Key Linked to Account #{sel.id}</span>
                            </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[11px] text-slate-700 dark:text-slate-300 pt-1">
                            <div><strong>Name:</strong> {sel.fullName || sel.name}</div>
                            <div><strong>Phone:</strong> {sel.phoneNumber || sel.phone || 'N/A'}</div>
                            <div><strong>Email:</strong> {sel.email}</div>
                          </div>
                          {enrolledWards.length > 0 && (
                            <div className="pt-1 border-t border-emerald-500/20 text-[11px] text-emerald-900 dark:text-emerald-200">
                              <strong>Enrolled Siblings in School:</strong>{' '}
                              {enrolledWards.map(w => `${w.firstName} ${w.lastName} (${w.currentClassArmName || w.admissionNumber})`).join(', ')}
                            </div>
                          )}
                        </div>
                      );
                ...
                <div className="space-y-3">
                  ...
                  {(() => {
                    const match = parentEmail.trim()
                      ? parents.find(p => p.email.toLowerCase() === parentEmail.trim().toLowerCase())
                      : null;
                    if (match) {
                      return (
                        <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-xs text-amber-900 dark:text-amber-200 flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2">
                            <AlertCircle className="w-4 h-4 text-amber-600 shrink-0" />
                            <span>
                              <strong>Account detected:</strong> {match.fullName} ({match.email}) already exists.
                            </span>
                          </div>
                          <button
                            type="button"
                            onClick={() => {
                              setParentMode('EXISTING');
                              handleSelectParent(match.id);
                            }}
                            className="px-2.5 py-1 rounded-lg bg-amber-500 text-slate-950 font-bold text-[11px] hover:bg-amber-400 cursor-pointer transition-colors"
                          >
        ...
        {submitError && (
          <div className="p-4 rounded-xl bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-xs text-red-700 dark:text-red-300 flex items-start gap-2.5">
            <AlertCircle className="w-4 h-4 shrink-0 mt-0.5 text-red-600 dark:text-red-400" />
            <div className="space-y-1">
              <div className="font-bold">Admission Registration Incomplete</div>
              <div>{submitError}</div>
            </div>
          </div>
        )}
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
          {step > 1 ? (
            <button
              ...
              className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-1.5 cursor-pointer transition-colors disabled:opacity-50"
            >
          ...
          {step < 4 ? (
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed text-white text-xs font-bold flex items-center gap-2 shadow-xs cursor-pointer transition-colors"
            >
              {isSubmitting ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                  <span>Enrolling Student into Database...</span>
                </>
              ) : (
                <>
